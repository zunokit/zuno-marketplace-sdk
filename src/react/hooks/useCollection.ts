...
export function useCollection() {
  ...
  const addToAllowlist = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: collectionQueryKeys.allowlist(variables.collectionAddress, ''),
      });
    },
  });
  ...
  const setAllowlistOnly = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: collectionQueryKeys.allowlistOnly(variables.collectionAddress),
      });
    },
  });
...
export function useCollectionInfo(address?: string) {
  ...
  return useQuery({
    ...collectionInfoQueryOptions(sdk, address!),
    enabled: !!address,
  });
}
...
export function useCreatedCollections(options?: {
  ...
  return useQuery({
    ...createdCollectionsQueryOptions(sdk, options),
...
export function useUserOwnedTokens(collectionAddress?: string, userAddress?: string) {
  ...
  return useQuery({
    ...userOwnedTokensQueryOptions(sdk, collectionAddress!, userAddress!),
...
export function useIsInAllowlist(collectionAddress?: string, userAddress?: string) {
  ...
  return useQuery({
    ...isInAllowlistQueryOptions(sdk, collectionAddress!, userAddress!),
    enabled: !!collectionAddress && !!userAddress,
  });
}
...
export function useIsAllowlistOnly(collectionAddress?: string) {
  ...
  return useQuery({
    ...isAllowlistOnlyQueryOptions(sdk, collectionAddress!),
...
export function useSetupAllowlist() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [...collectionQueryKeys.all, 'allowlist', variables.collectionAddress],
      });
      queryClient.invalidateQueries({
        queryKey: collectionQueryKeys.allowlistOnly(variables.collectionAddress),
      });
